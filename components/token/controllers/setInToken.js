 ...
 * @return address
 */
module.exports = async function setInToken(req, res) {
  ...
  try {
     ...
     */
    let { pin, address, privateKey } = req.body
    /**
     *
     * @desc — get pin from req.body
     */
    ...
    let addressOnToken = JSON.parse(isecSession?.value?.toString())?.address
    let privateKeyOnToken = JSON.parse(isecSession?.value?.toString())
      ?.privateKey
    if (!isecSession) {
